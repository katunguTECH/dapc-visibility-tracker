...
import { Check, Search, ArrowRight, MapPin, Share2, Users, Globe, AlertCircle, BarChart3 } from "lucide-react";
                ...
                {report.competitors.map((comp: any, i: number) => (
                  <div key={i}>
                    ...
                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                      <div className="bg-blue-600 h-full rounded-full transition-all duration-1000" style={{ width: `${comp.score}%` }}></div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {pricingPlans.map((plan, i) => (
              <div key={i} className="bg-white rounded-[2rem] p-6 border border-slate-200 flex flex-col items-center hover:border-blue-700 transition-all">
                ...
                {/* Only allow STK push if signed in */}
                {isSignedIn ? (
                  <MpesaModal amount={plan.price.replace(',', '')} planName={plan.name} />
                ) : (
                  <button
                    onClick={() => openSignIn?.({})}
                    className="bg-blue-700 text-white px-6 py-3 rounded-xl font-black hover:bg-black transition"
                  >
...
function Badge({ label, active }: { label: string; active: boolean }) {
  return (
    <span className={`text-[9px] px-2 py-1 rounded-md font-black uppercase tracking-widest border ${
      active ? "bg-green-50 text-green-700 border-green-100" : "bg-slate-50 text-slate-300 border-slate-100"
    }`}>
      {label}
    </span>
  );
