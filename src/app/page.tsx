...
const LeadCard = ({ icon: Icon, type, title, value, impact, color, onClick }: any) => (
  <div 
    onClick={onClick}
    className="bg-white p-8 rounded-[32px] border border-slate-100 hover:border-blue-600 hover:shadow-2xl transition-all duration-500 cursor-pointer group"
  >
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAFBFF] text-slate-900 selection:bg-blue-100">
      {/* NAV */}
      <nav className="flex justify-between items-center px-10 py-10 max-w-7xl mx-auto">
        <div className="font-black text-4xl text-blue-600 italic tracking-tighter cursor-pointer">DAPC</div>
        ...
        <button className="text-xs font-black uppercase bg-slate-900 text-white px-10 py-4 rounded-full hover:bg-blue-600 transition-all">
          Sign In
        </button>
      </nav>

      <main className="max-w-6xl mx-auto pt-16 pb-40 px-6">
        {/* HERO - FORCED SAAS LOOK */}
        <div className="text-center mb-16 space-y-8">
          <h1 
            style={{ letterSpacing: '-0.06em', lineHeight: '0.85' }} 
            className="text-6xl md:text-[110px] font-[900] text-slate-900"
          >
        ...
        {/* SEARCH BAR */}
        <div className="bg-white p-4 rounded-[40px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.08)] border border-slate-100 flex flex-col md:flex-row gap-3 mb-32">
          <input 
          ...
          />
          <button onClick={startAudit} disabled={loading} className="bg-blue-600 text-white px-14 py-8 rounded-[30px] font-black text-2xl hover:bg-blue-700 transition-all shadow-2xl shadow-blue-200">
        ...
        {data ? (
          <div className="animate-in fade-in slide-in-from-bottom-10 duration-700">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-20">
              {/* Vitals Card */}
              <div className="bg-white p-14 rounded-[60px] border border-slate-50 shadow-sm text-center">
                <span className="text-[11px] uppercase font-black text-slate-300 tracking-[0.4em] mb-6 block">Visibility Score</span>
                <div 
                   style={{ letterSpacing: '-0.05em' }}
                   className={`text-[150px] font-black leading-none ${data.score > 55 ? 'text-blue-600' : 'text-red-600'}`}
                >
                  {data.score}%
                </div>
                <div className="mt-8 inline-flex items-center gap-3 px-8 py-3 rounded-full text-xs font-black uppercase tracking-widest bg-slate-50">
              ...
              <div className="bg-white p-14 rounded-[60px] border border-slate-50 shadow-sm flex flex-col justify-center">
                <h2 className="text-5xl font-black text-slate-900 leading-tight mb-8 tracking-tighter">{data.businessName}</h2>
                <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-3 text-lg font-black text-slate-900 bg-slate-50 self-start px-6 py-3 rounded-2xl">
            ...
            <div id="leads-section" className="pt-20 border-t border-slate-100">
               <h2 className="text-5xl font-black text-slate-900 tracking-tight mb-12">Growth Leads</h2>
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                 <LeadCard icon={MessageCircle} type="Direct" title="WhatsApp" value="Verify Profile" impact="High" color="bg-green-500" onClick={() => window.open(`https://wa.me/254710440648?text=Audit for ${data.businessName}`, '_blank')} />
                 <LeadCard icon={Mail} type="Email" title="Outreach" value={data.leads?.email || "Scanning..."} impact="High" color="bg-blue-500" />
                 <LeadCard icon={Share2} type="Social" title="Footprint" value="FB/IG Audit" impact="Med" color="bg-purple-500" />
               </div>
            </div>
          </div>
        ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16 mt-20">
            <div className="group">
              <div className="w-16 h-16 bg-blue-50 rounded-[24px] flex items-center justify-center text-blue-600 mb-8 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500 shadow-sm">
                <ShieldCheck size={32} />
              </div>
