...
export default function LandingPage() {
  const { isLoaded, userId } = useAuth(); // Get Clerk auth state
  ...
  const handlePay = async (planName: string, amount: number) => {
    ...
    try {
      const res = await fetch("/api/mpesa/stk-push", {
        ...
        // 2. PASS USERID TO API (Crucial for tracking in DB)
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="mb-12 flex justify-center">
        <div className="w-full max-w-2xl flex gap-2">
          ...
          />
          <button onClick={handleSearch} className="bg-blue-600 text-white px-8 py-4 rounded-xl font-bold shadow-lg">
            Run Audit
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
        {plans.map((plan) => (
          <div key={plan.name} className="p-6 bg-white border border-gray-100 rounded-3xl shadow-lg flex flex-col items-center text-center">
            <img src={`/icons/${plan.icon}.jpg`} alt={plan.name} className="h-28 w-28 object-contain mb-4" />
