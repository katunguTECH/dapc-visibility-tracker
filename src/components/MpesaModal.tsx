...
export default function MpesaModal({ isOpen, onClose, planName, amount }: any) {
  ...
  const handleSTKPush = async () => {
    // PREVENT API CALL IF NUMBER IS MISSING
    if (!phone || phone.length < 10) {
      alert("Please enter your Safaricom number first.");
      return;
    }
    ...
    try {
      ...
      alert("API Error. Check your connection.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white w-full max-w-sm rounded-[2rem] p-10 shadow-2xl relative border-2 border-gray-100">
        ...
        <div className="mb-8 text-left">
          ...
          <input
            type="tel"
            autoFocus
            className="w-full p-5 bg-gray-50 border-2 border-gray-100 rounded-2xl font-black text-xl outline-none focus:border-green-500 text-black"
        ...
        <button 
          ...
          disabled={loading}
          className="w-full py-5 bg-green-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-black transition-all disabled:opacity-50"
        >
